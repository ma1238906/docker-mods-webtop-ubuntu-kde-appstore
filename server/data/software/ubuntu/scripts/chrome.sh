# sudo -E bash ./install_chrome.deb
#!/usr/bin/env bash


# export PROXY="http://192.168.2.19:7897"      # HTTP 代理
...
if [[ "${DISTRO}" == @(centos|oracle8|rockylinux9|rockylinux8|oracle9|rhel9|almalinux9|almalinux8) ]]; then
  ...
  if [ ! -z "${CHROME_VERSION}" ]; then
    ...
    wget http://192.168.2.239:8081/repository/apt-internal/pool/g/google-chrome-stable/google-chrome-stable_141.0.7390.122-1_amd64.deb -O chrome.deb
  fi
